...
if [ "$DO_BUILD" = "1" ] || [ "$DO_BUILD" = "true" ]; then
    ...
    # --editable: the source tree is regenerated by generate.sh on every run, so a
    # cached binary package is always stale. Disables binary caching so vcpkg
    # always does a real (incremental) rebuild of the gdext port.
