...
func test_brepmesh_triangle_ctor() -> String:
	...
	var tri := OcgBrepmeshTriangle.from_pdl(edges, orientations, OcgEnums.BRepMesh_DegreeOfFreedom.BRepMesh_Free)
	if tri == null:
		return "from_pdl failed"
...
func test_poly_mergenodes_addtriangle() -> String:
	var tool := OcgPolyMergenodestool.from_Nj0(30.0, 1e-6, 100)
	if tool == null:
		return "from_Nj0 failed"
	var p0 := OcgGpXYZ.from_668(0.0, 0.0, 0.0)
	var p1 := OcgGpXYZ.from_668(1.0, 0.0, 0.0)
	var p2 := OcgGpXYZ.from_668(0.0, 1.0, 0.0)
	var p3 := OcgGpXYZ.from_668(1.0, 1.0, 0.0)
